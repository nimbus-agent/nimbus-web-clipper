...
- Is it safe to ship? — on a pull request or a build, the panel answers with a deploy verdict over three checks your gateway already computes: active P1 incidents, failing CI runs and open merge conflicts, with each finding linking to the thing it names. The first time it sees a repository it asks which Nimbus service that is, seeded with a guess rather than guessing silently, and checks the id against your gateway before saving it.
