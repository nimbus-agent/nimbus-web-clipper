...
You pick one. Free text exists behind a disclosure as an escape hatch — the surface leads
with what it already knows, and typing is the fallback rather than the entry.
...
it: the source is cut to `MAX_SOURCE_BYTES`, `truncated: true` goes on the payload, and the
composer names the shortened sources in the result.
...
Bounded like every other store in this repo: a cap on retained entries, oldest evicted
first, and a user-facing control to clear it. The eviction is stated in the UI rather than
silent, because a log that quietly forgets is worse than one that says it only keeps the
last N.
...
Feeding is **sequential**: it gives honest `received`/`expected` progress and fits the
bucket the gateway sized for it. `brief-src` is its own rate-limit bucket at 60/min,
commented upstream as ensuring *"a sweep feeding up to 20 sources back-to-back cannot"*
starve ordinary clipping. Create, run and save share a separate `brief` bucket.

Polling reuses the agent-lane pattern exactly: a `setTimeout` backoff for the live cadence,
with `chrome.alarms` as the eviction net only. The one-minute alarm floor matters more here
than it does for agent lanes — synthesis over up to 4 MB of source text can genuinely
outlast a service worker — so the net is likelier to actually fire.
...
| 413 | That source only — truncate, flag, keep the run | n/a |
...
Two assertions worth writing as tests rather than trusting to review: that no storage write
in this slice contains a source body, and that a log entry is written on a run that
subsequently **fails**. Both are decisions above that a later refactor could quietly undo.
