...
- **Add a surface recogniser** — a new product module under
  ...
  from a separate table anymore.
  **Corrected a third time — count honestly, not the happy path.** This bullet
  was wrong once and was fixed in Slice 1, when it still named `PRODUCT_CORPUS`
  and a hardcoded `<option>` list — the two tables Slice 1 itself deleted. What
  Slice 3's Task 2 found once those were gone: for an `origin`-kind host (a SaaS
  product recognised on a fixed hostname — every shipped product except Jira,
  which is `suffix`-kind), the module/registry/id above is not the whole cost.
  It also forces a conscious edit to **seven literal test lists**, pinned on
  purpose so a product cannot land silently under-tested: `BUILT_IN_ORIGINS`,
  `BUILT_IN_SURFACES`, its built-in-row count, and `PRODUCT_SERVICE_ID` (all in
  ...
  `options.test.ts`. Still the cleanest entry point into the reframe — just not
  a four-place one.
